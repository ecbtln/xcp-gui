...
from . import PyQtGui, PY_QT5

class AssetExchange(PyQtGui.QWidget):
    def __init__(self, *args, **kwargs):
        super(AssetExchange, self).__init__(*args, **kwargs)

        glob_vbox_layout = PyQtGui.QVBoxLayout()
        place_new_order = PyQtGui.QPushButton("Place Order")
        ...
        do_btc_pay = PyQtGui.QPushButton("BTC Pay")
        cancel_order = PyQtGui.QPushButton("Cancel Order")
        # asset_lookup = QPushButton("Lookup Asset")
        button_box = PyQtGui.QDialogButtonBox()
        button_box.addButton(place_new_order, PyQtGui.QDialogButtonBox.NoRole)
        button_box.addButton(do_btc_pay, PyQtGui.QDialogButtonBox.NoRole)
        button_box.addButton(cancel_order, PyQtGui.QDialogButtonBox.NoRole)
        ...
        group_box = PyQtGui.QGroupBox('Open Orders (Double click to cancel)')
        ...
        group_box = PyQtGui.QGroupBox('Order Matches (Double click to BTCpay)')
        vbox_layout = PyQtGui.QVBoxLayout()
    ...
    def fetch_matched_orders(self):
        ...
        def callback(matched_orders):
            ...
            for i, o in enumerate(matched_orders):
                self.order_matches.setItem(i, 0, PyQtGui.QTableWidgetItem('%d' % o['tx0_index']))
                ...
                self.order_matches.setItem(i, 1, PyQtGui.QTableWidgetItem(o['tx0_address']))
                self.order_matches.setItem(i, 2, PyQtGui.QTableWidgetItem('%s %s' % (forward_amount, forward_asset)))
                self.order_matches.setItem(i, 3, PyQtGui.QTableWidgetItem('%d' % o['tx1_index']))
                self.order_matches.setItem(i, 4, PyQtGui.QTableWidgetItem(o['tx1_address']))
                self.order_matches.setItem(i, 5, PyQtGui.QTableWidgetItem('%s %s' % (backward_amount, backward_asset)))
                self.order_matches.setItem(i, 6, PyQtGui.QTableWidgetItem(o['validity']))
    ...
    def fetch_open_orders(self):
        ...
        def callback(orders):
            ...
            btc = PyQtGui.QApplication.instance().wallet.get_asset(BTC)
            for i, o in enumerate(orders):
                self.open_orders.setItem(i, 0, PyQtGui.QTableWidgetItem('%d' % o['tx_index']))
                get_asset = o['get_asset']
                get_asset_obj = app.wallet.get_asset(get_asset)
                assert get_asset_obj is not None
                ...
                self.open_orders.setItem(i, 1, PyQtGui.QTableWidgetItem('%s %s' % (get_amount, get_asset)))
                self.open_orders.setItem(i, 2, PyQtGui.QTableWidgetItem('%s %s' % (give_amount, give_asset)))
                self.open_orders.setItem(i, 3, PyQtGui.QTableWidgetItem('%.2f %s/%s' % (float(give_amount)/float(get_amount), give_asset, get_asset)))
                self.open_orders.setItem(i, 4, PyQtGui.QTableWidgetItem('%d' % (o['block_index'] + o['expiration'])))

                self.open_orders.setItem(i, 5, PyQtGui.QTableWidgetItem(str(btc.convert_for_app(o['fee_required']))))
                self.open_orders.setItem(i, 6, PyQtGui.QTableWidgetItem(str(btc.convert_for_app(o['fee_provided']))))
                self.open_orders.setItem(i, 7, PyQtGui.QTableWidgetItem('%s %s' %
...
class OrderMatchesTableView(PyQtGui.QTableWidget):
...
class OpenOrdersTableView(PyQtGui.QTableWidget):
    def __init__(self, *args):
...
class PlaceOrderDialog(PyQtGui.QDialog):
    def __init__(self, asset, *args, **kwargs):
        ...
        self.give_combo_box = PyQtGui.QComboBox()
        self.give_combo_box.setToolTip("The asset to give")
        self.wallet = PyQtGui.QApplication.instance().wallet
        ...
        form_layout.addRow("Get Asset: ", PyQtGui.QLabel(self.get_asset))
        ...
        button_box.addButton("Order", PyQtGui.QDialogButtonBox.AcceptRole)
    ...
    def submit(self):
        ...
        PyQtGui.QApplication.instance().xcp_client.do_order(source, give_quantity, give_asset, get_quantity, get_asset,
...
class PickGetAssetOrderDialog(PyQtGui.QDialog):
    def __init__(self, *args, **kwargs):
        ...
        self.submit_button = PyQtGui.QPushButton("Select")
        button_box.addButton(self.submit_button, PyQtGui.QDialogButtonBox.AcceptRole)
        form_layout.addRow(button_box)
        button_box.rejected.connect(self.close)
        button_box.accepted.connect(self.submit)
        self.setLayout(form_layout)
...
class CancelOrderDialog(PyQtGui.QDialog):
    def __init__(self, tx_hash=None, *args, **kwargs):
        ...
        self.offer_hash = PyQtGui.QLineEdit()
        if tx_hash:
        ...
        button_box = PyQtGui.QDialogButtonBox()
        button_box.addButton("Undo", PyQtGui.QDialogButtonBox.RejectRole)

        self.submit_button = PyQtGui.QPushButton("Submit Request")
        button_box.addButton(self.submit_button, PyQtGui.QDialogButtonBox.AcceptRole)
        form_layout.addRow(button_box)
        button_box.rejected.connect(self.close)
        button_box.accepted.connect(self.submit)
    ...
    def submit(self):
        def success_callback(response):
            ...
            ShowTransactionDetails(response).exec_()
        self.close()
        PyQtGui.QApplication.instance().xcp_client.do_cancel(self.offer_hash.text(), success_callback)

    def textChanged(self):
        self.submit_button.setEnabled(len(self.offer_hash.text()) > 0)


class BTCPayDialog(PyQtGui.QDialog):
    def __init__(self, order_match_id=None, amt=None, *args, **kwargs):
        ...
        if amt is None:
            ...
            a = PyQtGui.QApplication.instance().wallet.get_asset(BTC)
            self.setWindowTitle("Send %s %s" % (a.convert_for_app(amt), BTC))

        form_layout = PyQtGui.QFormLayout()

        self.order_match_id = PyQtGui.QLineEdit()
        if order_match_id:
        ...
        button_box = PyQtGui.QDialogButtonBox()
        button_box.addButton("Cancel", PyQtGui.QDialogButtonBox.RejectRole)
        self.submit_button = PyQtGui.QPushButton("Pay")
        button_box.addButton(self.submit_button, PyQtGui.QDialogButtonBox.AcceptRole)
        form_layout.addRow(button_box)
        button_box.rejected.connect(self.close)
        button_box.accepted.connect(self.submit)
    ...
    def submit(self):
        ...
        PyQtGui.QApplication.instance().xcp_client.do_btcpay(self.order_match_id.text(), success_callback)
        self.close()
